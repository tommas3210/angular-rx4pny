...
const ADD_CONFIG = {
    ...
    COMPILED_THEME_PATH: 'node_modules/ng-quicksilver/ng-quicksilver.min.css',
    BOOT_PAGE_PATH: 'src/app/app.component.html',
    BOOT_PAGE_HTML: `<!-- WEBDPT -->
...
function addI18n(options) {
    return function (host) {
        ...
        const changes = [
            route_utils_1.insertImport(moduleSource, modulePath, 'DW_I18N', 'ng-quicksilver'),
            route_utils_1.insertImport(moduleSource, modulePath, locale, 'ng-quicksilver'),
...
function insertI18nTokenProvide(moduleSource, modulePath, locale) {
    ...
    const addProvide = ast_utils_1.addSymbolToNgModuleMetadata(moduleSource, modulePath, 'providers', `{ provide: DW_I18N, useValue: ${locale} }`, null);
    ...
    if (matchingProperties.length) {
        ...
        if (arrLiteral.elements.length === 0) {
            ...
            node = arrLiteral.elements.filter(e => e.getText && e.getText().includes('DW_I18N'));
            if (node.length === 0) {
                return addProvide;
            }
            return node.map(e => new change_1.ReplaceChange(modulePath, e.getStart(), e.getText(), `{ provide: DW_I18N, useValue: ${locale} }`));
        }
    }
    else {
        return addProvide;
...
/** 添加 ng-quicksilver 到 package.json 的 dependencies */
function addZorroToPackageJson() {
    return (host) => {
        package_1.addPackageToPackageJson(host, 'dependencies', 'ng-quicksilver', lib_versions_1.zorroVersion);
        return host;
    };
}
/** 添加 BrowserAnimationsModule FormsModule HttpClientModule NgQuicksilverModule 到 app.module */
function addModulesToAppModule(options) {
    return (host) => {
        ...
        ast_1.addModuleImportToRootModule(host, 'NgQuicksilverModule', 'ng-quicksilver', project);
        return host;
    };
}
/** 添加样式配置 */
